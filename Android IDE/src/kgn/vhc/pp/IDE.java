...
import java.io.*; 
import kgn.vhc.pp.libs.lib.io.StringWriterOutputStream;
...
//      System.out.println(G.Rstring(R.string.msg_aapt_warning));
    	JavaFile.fnLog("i","WARNING: Aapt was built for Android API level 9 or higher.");
    	JavaFile.fnLog("i","Your API level is:"+" "+apiLevel);
    	JavaFile.fnLog("i","Aapt might not work correctly on your device!");
    }
    
    /*if (JavaFile.fnCheckWorkDir(false)==false)
    {
      JavaFile.fnLog("i","Mandatory working directory /sdcard/.JavaIDEdroid/ does not exist.");
      return 99;
    }*/
...
//===================================================================
{
  ...
  {
    if (swos==null) return;
    ok = JavaFile.fnMakeLogDir(false);
    if (!ok)
    {
//      System.err.println(G.Rstring(R.string.err_mkdir)+" "+G.stWorkDir);
    ...
    fw = new FileWriter (""+"LogOutput.txt",false);
